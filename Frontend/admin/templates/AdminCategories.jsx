...
// API base (absolute) to avoid proxy/network ambiguity during dev
const API_BASE = 'http://localhost:8080';

// Simple API helpers using fetch with robust error handling
...
const CategoriesSection = () => {
  ...
  const handleAddCategory = async () => {
    try {
      await api('/api/admin/categories', {
        method: 'POST',
        body: JSON.stringify({ name: modalCategoryName })
      });
      fetchCategories();
      closeModal();
    } catch (error) {
  ...
  const handleUpdateCategory = async () => {
    try {
      await api(`/api/admin/categories/${selectedCategoryId}`, {
        method: 'PUT',
        body: JSON.stringify({ name: modalCategoryName })
      });
      fetchCategories();
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      if (formData.category) {
        categoryObj = categories.find(c => c.name === formData.category);
      }
      ...
      let created = null;
      if (editingProductId) {
        // Update existing product with JSON
        const data = await api(`/api/admin/products/${editingProductId}`, {
          method: 'PUT',
          body: JSON.stringify(payload)
        });
        created = data;
        console.log('[PRODUCT] Updated:', created);
      } else {
        // Create new product with JSON
        const data = await api('/api/admin/products', {
          method: 'POST',
          body: JSON.stringify(payload)
        });
      ...
      // If admin selected images in the modal, upload them separately
      if (formData.images && formData.images.length > 0 && created && created.id) {
        const fd = new FormData();
        formData.images.forEach(img => fd.append('images', img));

        const uploadRes = await fetch(`${API_BASE}/api/admin/products/${created.id}/images`, {
          method: 'POST',
          body: fd,
        });
      ...
      console.error('[ERROR] Save product error:', error);
            ...
            <div className="product-actions">
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  try {
                    const r = await fetch(`${API_BASE}/api/admin/products/${product.id}/images`, {
                      method: 'POST',
                      body: fd,
                    });
                    if (!r.ok) {
      ...
      {showModal && (
        ...
        >
            ...
            <div className="modal-header">
                <h2>{editingProductId ? `Edit Product (ID: ${editingProductId})` : 'Add New Product'}</h2>
